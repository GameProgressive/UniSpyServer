...
namespace UniSpy.Server.NatNegotiation.Test
{
    public class GameTest
    {
        ...
        {
            new InitPacketCache.RedisClient().Db.Execute("FLUSHALL");
        ...
        {
            new InitPacketCache.RedisClient().Db.Execute("FLUSHDB");
            var clients = new Dictionary<string, IClient>()
            {
                {"client1gp",MockObject.CreateClient("192.168.0.109",1111)},
                {"client1nn",MockObject.CreateClient("192.168.0.109",64900)},
        ...
        {
            new InitPacketCache.RedisClient().FlushDb();
        ...
        [Fact]
        public void MarvelPs2_20230707()
        {
            var client1GP = MockObject.CreateClient("192.168.9.2", 5165);
            var client1NN = MockObject.CreateClient("192.168.9.2", 59300);
            var client2GP = MockObject.CreateClient("192.168.9.9", 5165);
            var client2NN = MockObject.CreateClient("192.168.9.9", 62545);

            var requests = new List<KeyValuePair<ITestClient, byte[]>>()
            {
                new KeyValuePair<ITestClient, byte[]>(client1GP,new byte[]{0xFD,0xFC,0x1E,0x66,0x6A,0xB2,0x02,0x00,0x7D,0x47,0xCF,0x14,0x00,0x01,0x01,0xC0,0xA8,0x09,0x02,0x00,0x00,0x6D,0x61,0x72,0x76,0x6C,0x65,0x67,0x70,0x73,0x32,0x70,0x00,0x00}),
                new KeyValuePair<ITestClient, byte[]>(client1NN,new byte[]{0xFD,0xFC,0x1E,0x66,0x6A,0xB2,0x02,0x00,0x7D,0x47,0xCF,0x14,0x01,0x01,0x01,0xC0,0xA8,0x09,0x02,0x00,0x00,0x6D,0x61,0x72,0x76,0x6C,0x65,0x67,0x70,0x73,0x32,0x70,0x00}),
                new KeyValuePair<ITestClient, byte[]>(client1NN,new byte[]{0xFD,0xFC,0x1E,0x66,0x6A,0xB2,0x02,0x00,0x7D,0x47,0xCF,0x14,0x02,0x01,0x01,0xC0,0xA8,0x09,0x02,0x14,0x2D,0x6D,0x61,0x72,0x76,0x6C,0x65,0x67,0x70,0x73,0x32,0x70,0x00}),
                new KeyValuePair<ITestClient, byte[]>(client2GP,new byte[]{0xFD,0xFC,0x1E,0x66,0x6A,0xB2,0x02,0x00,0x7D,0x47,0xCF,0x14,0x00,0x00,0x01,0xC0,0xA8,0x09,0x09,0x00,0x00,0x6D,0x61,0x72,0x76,0x6C,0x65,0x67,0x70,0x73,0x32,0x70,0x00}),
                new KeyValuePair<ITestClient, byte[]>(client2NN,new byte[]{0xFD,0xFC,0x1E,0x66,0x6A,0xB2,0x02,0x00,0x7D,0x47,0xCF,0x14,0x02,0x00,0x01,0xC0,0xA8,0x09,0x09,0x14,0x2D,0x6D,0x61,0x72,0x76,0x6C,0x65,0x67,0x70,0x73,0x32,0x70,0x00}),
                new KeyValuePair<ITestClient, byte[]>(client2NN,new byte[]{0xFD,0xFC,0x1E,0x66,0x6A,0xB2,0x02,0x00,0x7D,0x47,0xCF,0x14,0x01,0x00,0x01,0xC0,0xA8,0x09,0x09,0x00,0x00,0x6D,0x61,0x72,0x76,0x6C,0x65,0x67,0x70,0x73,0x32,0x70,0x00})
            };
