...
namespace UniSpy.Server.ServerBrowser.V1.Aggregate
{
    public class Enctype1 : EnctypeBase, IEnctype1Test
    {
        ...
        private uint[] _tbuff = new uint[326];
        ...
        public override byte[] Encrypt(byte[] data)
        {
            ...
            if (data.Length! > 1)
            {
                throw new ServerBrowser.Exception("The encryption data can not be empty");
            }
            int tempLen = (data.Length >> 1) - 17;
            int encshare4Data = 0;
            byte[] encshare4DataBytes = BitConverter.GetBytes(encshare4Data);
            if (tempLen >= 0)
            {
                EncShare4(encshare4DataBytes, encshare4DataBytes.Length, _tbuff);
                Encshare1(_tbuff, 0, data, data.Length, tempLen);
            }
            var scrambleData = Enumerable.Repeat<byte>(0x01, 16);
            var encryption_key = new byte[258];
            // Func3()
            var output = new uint[data.Length / sizeof(byte)];
            var outputBytes = output.SelectMany(BitConverter.GetBytes).ToArray();
            if (tempLen >= 0)
            {
                EncShare4(dataCopy, ClientKey.Length, output);
                Func6(dataCopy, tempLen);
            }
            var newOutput = new List<byte>();
            // 4 bytes message length
            // newOutput.AddRange(new byte[4] { 0x00, 0x00, 0x00, 0x00 });
            newOutput.Add(42);
            newOutput.Add(218);
            // 6 to 19 is unknown data, we do not use it
            newOutput.AddRange(Enumerable.Repeat<byte>(0, 13));
            newOutput.AddRange(scrambleData);
            // unused data
            newOutput.AddRange(new byte[4] { 0x00, 0x00, 0x00, 0x00 });
            // encshare4data
            newOutput.AddRange(new byte[4] { 0x00, 0x00, 0x00, 0x00 });
            // unused data
            newOutput.AddRange(Enumerable.Repeat<byte>(0x00, 18));

            newOutput.AddRange(dataCopy);
            // insert encryption length to index 0
            newOutput.InsertRange(0, BitConverter.GetBytes(newOutput.Count).Reverse());
