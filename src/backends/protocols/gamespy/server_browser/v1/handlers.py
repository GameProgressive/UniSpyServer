...
from backends.protocols.gamespy.server_browser.v1.responses import (
    GroupListResponse,
    ServerInfoResponse,
...
class ServerInfoHandler(HandlerBase):
    ...
    def _result_construct(self) -> None:
        self._result = ServerInfoResult(servers=self._caches)
...
class ServerListCompressHandler(HandlerBase):
    ...
    def _result_construct(self) -> None:
        self._result = ServerListCompressResult(servers=self._caches)
...
class GroupListHandler(HandlerBase):
    ...
    def _result_construct(self) -> None:
        self._result = GroupListResult(peer_rooms=self._caches)
