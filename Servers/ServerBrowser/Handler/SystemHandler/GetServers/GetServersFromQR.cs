...
namespace ServerBrowser.Handler.CommandHandler.ServerList.GetServers
{
    public class GetServersFromQR
    {
        ...
        public byte[] GenerateServerKeys(ServerListPacket packet)
        {
            List<byte> data = new List<byte>();
            switch (packet.UpdateOption)
            {
                case SBServerListUpdateOption.SendRequestedField:
                    //add requested keys to list
                    _totalKeysNumber = packet.FieldList.Length;
                    data.Add((byte)_totalKeysNumber);
                    //The following byte should be keyType: maybe serverkey playerkey teamkey
                    foreach (var standardKey in packet.FieldList)
                    {
        ...
        public byte[] GenerateServerInfos(ServerListPacket packet)
        {
            List<byte> data = new List<byte>();
            foreach (var server in _filteredServers)
            {
                ...
                switch (packet.UpdateOption)
                {
                    case SBServerListUpdateOption.SendRequestedField:
                        ...
                        foreach (var key in packet.FieldList)
                        {
                            data.Add(0xFF);
