...
namespace PresenceConnectionManager.Handler.General.Login
{
    ...
    public class LoginQuery
    {
        ...
        {
            var result = GPCMServer.DB.Query(@"SELECT users.userid,profiles.profileid, users.password, users.emailverified, users.banned"
            + @" FROM profiles INNER JOIN users ON profiles.userid = users.userid "
            + @"WHERE profiles.nick = @P0 AND users.email=@P1", nickname, email);
            return (result.Count == 0) ? null : result;
        }
